...
while(round < 11){
  ...
  for(i in 1:length(unique(data$Date))){
    ...
    for(j in 1:length(unique(subset_date$Altitude))){
      ...
      colnames(tree_data_alt_tocol)[4:ncol(tree_data_alt_tocol)] <- paste(colnames(tree_data_alt_tocol)[4:ncol(tree_data_alt_tocol)], 
                                                                          current_alt, sep = "_")
      ...
      if(round == 1){
        ...
        # Plot tree
        setwd(paste(fig.dir, "Trees/ctree/80-20split_altsingle", sep = "/"))
        
        png(paste(current_date, alt, "ctree", ".png", sep="_"), width = 16, height = 8, units = 'in', res = 300)
        
        plot(validated_tree$finalModel, 
             main = paste(current_date, alt, sep = " "))
        dev.off()
        
        png(paste(current_date, alt, "ctree_PC", ".png", sep="_"), width = 16, height = 8, units = 'in', res = 300)
    ...
    if(length(unique(subset_date$Altitude)) >1){
      ...
      if(round == 1){
        ...
        # Plot tree
        setwd(paste(fig.dir, "Trees/ctree/80-20split_altcombi", sep = "/"))
...
setwd(paste(out.dir, "ctree/results", sep = "/"))
write.csv(results_df, "ctree_cv_10_10_8020split.csv")
  ...
  for(i in 1:length(unique(data$Date))){
    ...
    for(j in 1:length(unique(subset_date$Altitude))){
      ...
      if(round == 1){
        ...
        # Plot tree
        setwd(paste(fig.dir, "Trees/ctree/nosplit_onlytrain_altsingle", sep = "/"))
    ...
    if(length(unique(subset_date$Altitude)) >1){
      ...
      if(round == 1){
        ...
        # Plot tree
        setwd(paste(fig.dir, "Trees/ctree/nosplit_onlytrain_altcombi", sep = "/"))
...
setwd(paste(out.dir, "ctree/results", sep = "/"))
write.csv(results_df, "ctree_cv_10_10_nosplit_onlytrain.csv")
